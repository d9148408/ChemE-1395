...
步驟 4：教學案例演練（例 MSCCE-1）
  └─ 依影片教學邏輯，逐步建立 4 種方案（Decanter 單級／錯流／逆流、EXTRACT 模塊）並比較分離效果
```

### 本單元互動學習資源

本單元配套有 **[MSCCE 互動教學網頁](MSCCE_Interactive.html)**，共 4 個互動頁籤，建議完成各節理論後對應開啟：
...
> 🎮 **互動教學工具**：開啟 [MSCCE 互動教學網頁](MSCCE_Interactive.html) 的「**① 操作模式比較**」頁籤，拖動 $E$ 與 $N$ 滑桿，即時比較單級、錯流與逆流三種模式的回收率，並觀察各模式流程示意圖（錯流：各級新鮮溶劑；逆流：同批溶劑逆向穿越全塔）。
...
> 🎮 **互動教學工具**：開啟 [MSCCE 互動教學網頁](MSCCE_Interactive.html) 的「**② Y-X 圖解互動**」頁籤，調整 KLL、萃取因子 $E$、 $X_F$ 與目標回收率，即時觀察逐板階梯動畫與夾緊點位置，直觀理解 McCabe-Thiele 圖解法；點選「▶ 逐步動畫」可逐格查看每板的水平與垂直步驟。
...
> 🎮 **互動教學工具**：開啟 [MSCCE 互動教學網頁](MSCCE_Interactive.html) 的「**③ Kremser 計算器**」頁籤，輸入設計條件（KLL、 $X_F$、 $X_R$、 $F'$、 $S'$），即時計算理論板數 $N$、 $S'_{\min}$ 與 $E_{\min}$，並顯示回收率隨 $N$ 變化的敏感度曲線。點選「載入 MSCCE-1 例題」可一鍵帶入本節例題數據驗算。
...
> 🎮 **互動教學工具**：開啟 [MSCCE 互動教學網頁](MSCCE_Interactive.html) 的「**④ 逐板濃度動畫**」頁籤，調整 KLL、萃取因子 $E$、 $X_F$ 與板數 $N$，動態觀察萃余相（X 相）與萃取相（Y 相）在各板的濃度分布，加深對「逐板傳質梯度」的直觀理解；各板組成同步以橫條圖與數值列表呈現。
...
**題目**：進料流量 100 kg/h（EA 70 wt%，丙酮 30 wt%），以純水（100 kg/h）作萃取劑，操作條件 25°C、1 atm。依照影片（KG Engineering Solutions）的教學邏輯，依序以 Aspen Plus 建立以下四種方案並比較萃取效果：
