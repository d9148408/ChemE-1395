...
步驟 4：教學案例演練（例 MSCCE-1）
  └─ 逐步建立 4 種方案（Decanter 單級／錯流／逆流、EXTRACT 模塊）並比較分離效果
```

### 本單元互動學習資源

本單元配套有 **[MSCCE 互動教學網頁](MSCCE_Interactive.html)（需下載至本機使用）**（HTML 互動網頁），共 4 個互動頁籤，建議完成各節理論後對應開啟：

> ⚠️ **使用方式（重要）**：在 GitHub 上點擊此連結會顯示 HTML 原始碼。請先**右鍵另存新檔**（或 Clone 整個 Repository）將 MSCCE_Interactive.html 存至本機，再以瀏覽器（Chrome / Edge / Firefox）直接開啟即可正常使用。

| 頁籤 | 主題 | 對應章節 |
|:---:|:---|:---:|
| ① 操作模式比較 | 即時比較三種操作模式回收率；流程示意圖 | §MSCCE.1.4 |
| ② Y-X 圖解互動 | McCabe-Thiele 逐板階梯動畫；夾緊點視覺化 | §MSCCE.2.5 |
| ③ Kremser 計算器 | 理論板數、最小溶劑量直接計算；敏感度曲線 | §MSCCE.2.7 |
| ④ 逐板濃度動畫 | 各板 X/Y 濃度動態分布；塔板傳質梯度視覺化 | §MSCCE.3 |

> **建議使用方式**：閱讀各節理論後，立即開啟對應頁籤互動驗算，將抽象公式轉化為直觀的視覺體驗。
...
> 🎮 **互動教學工具**：開啟 [MSCCE 互動教學網頁](MSCCE_Interactive.html)（需下載至本機使用） 的「**① 操作模式比較**」頁籤，拖動 $E$ 與 $N$ 滑桿，即時比較單級、錯流與逆流三種模式的回收率，並觀察各模式流程示意圖（錯流：各級新鮮溶劑；逆流：同批溶劑逆向穿越全塔）。
...
> 🎮 **互動教學工具**：開啟 [MSCCE 互動教學網頁](MSCCE_Interactive.html)（需下載至本機使用） 的「**② Y-X 圖解互動**」頁籤，調整 KLL、萃取因子 $E$、 $X_F$ 與目標回收率，即時觀察逐板階梯動畫與夾緊點位置，直觀理解 McCabe-Thiele 圖解法；點選「▶ 逐步動畫」可逐格查看每板的水平與垂直步驟。
...
> 🎮 **互動教學工具**：開啟 [MSCCE 互動教學網頁](MSCCE_Interactive.html)（需下載至本機使用） 的「**③ Kremser 計算器**」頁籤，輸入設計條件（KLL、 $X_F$、 $X_R$、 $F'$、 $S'$），即時計算理論板數 $N$、 $S'_{\min}$ 與 $E_{\min}$，並顯示回收率隨 $N$ 變化的敏感度曲線。點選「載入 MSCCE-1 例題」可一鍵帶入本節例題數據驗算。
...
> | 廢水–苯酚–MIBK（Unit07 例 7-3） | 水（進料，重） | MIBK（萃取劑，輕） | FEED(H2O)→板 1；SOLVENT(MIBK)→板 $N$ |
...
> 🎮 **互動教學工具**：開啟 [MSCCE 互動教學網頁](MSCCE_Interactive.html)（需下載至本機使用） 的「**④ 逐板濃度動畫**」頁籤，調整 KLL、萃取因子 $E$、 $X_F$ 與板數 $N$，動態觀察萃余相（X 相）與萃取相（Y 相）在各板的濃度分布，加深對「逐板傳質梯度」的直觀理解；各板組成同步以橫條圖與數值列表呈現。
...
**題目**：進料流量 100 kg/h（EA 70 wt%，丙酮 30 wt%），以純水（100 kg/h）作萃取劑，操作條件 25°C、1 atm。依序以 Aspen Plus 建立以下四種方案並比較萃取效果：

- **方案一**：單一 Decanter（單級萃取）
- **方案二**：3 個 Decanter 錯流萃取（水三等份，各級加入新鮮水）
- **方案三**：3 個 Decanter 逆流萃取（同一批水逆向流過三個 Decanter）
- **方案四**：EXTRACT 模塊 3 板逆流萃取（與方案三數學等價，但設定更簡潔）

**求**：各方案下 RAFFINATE 中丙酮的質量分率、丙酮進入水相（EXTRACT）的切割分率，並比較四種配置的分離效果差異。
...
> **溶質（丙酮）的分配方向**：在**稀溶液條件**下，丙酮的液液分配係數 $KLL_{\text{Acetone}} = x_{\text{Acetone}}^{\text{water}} / x_{\text{Acetone}}^{\text{EA}} \approx 1.2 > 1$ ，丙酮傾向分配至水相，因此水萃取可將丙酮從有機 EA 相帶走，使 EA 相得到濃縮提純。**但本案例進料丙酮高達 30 wt%，屬高濃度非稀溶液體系；在此濃度範圍，由於活度係數顯著偏離稀溶液假設，實際有效 $KLL$ 會大幅低於稀溶液極限值**（詳見「設計前分析」小節的量化說明）。
...
若假設丙酮濃度極低（稀溶液），NRTL-LLE 模型在 25°C 條件下的液液分配係數稀溶液極限值 $KLL_{\text{Acetone}}^{\infty} \approx 1.15 \sim 1.3$ 。以此估算萃取因子：

$$
F' = \frac{70\,\text{kg/h}}{88\,\text{g/mol}} = 0.795\,\text{kmol/h}, \qquad S' = \frac{100\,\text{kg/h}}{18\,\text{g/mol}} = 5.56\,\text{kmol/h}
$$

$$
E_{\text{dilute}} = KLL^{\infty} \times \frac{S'}{F'} \approx 1.2 \times \frac{5.56}{0.795} \approx 8.4
$$

| 方案 | 配置 | 板數 $N$ | 預估丙酮回收率（稀溶液 $E = 8.4$ ） |
|:---:|:---:|:-------:|:----------------------------------:|
| 方案一 | 單一 Decanter | 1 | $E/(1+E) \approx 89.4\%$ |
| 方案二 | 3 Decanter 錯流 | — | $1-(3/4.5)^3 \approx 70.4\%$（同 $E=1.5$ 公式**不適用**此體系） |
| 方案三/四 | 3 板逆流 | 3 | $E(E^3-1)/(E^4-1) \approx 99.9\%$ |

> **⚠️ 稀溶液假設對本體系完全失效——以下說明原因。**

---

#### 高濃度效應：為何稀溶液估算嚴重高估分離效果？

本案例進料為 **EA 70 wt%＋丙酮 30 wt%**，丙酮質量分率高達 30 wt%（工業上已屬高濃度進料）。**稀溶液假設要求丙酮濃度趨近於零**，因此稀溶液極限 $KLL^{\infty} \approx 1.2$ 完全不適用於本體系的操作濃度。

高濃度下導致 $KLL$ 大幅下降的物理機制：

1. **活度係數強烈偏離 1**：30 wt% 丙酮使體系進入高非理想性濃度範圍，兩相的活度係數 $\gamma$ 大幅偏離稀溶液極限值
2. **EA 與水的互溶度增加**：高濃度丙酮作為共溶劑（co-solvent），使原本部分互溶的 EA–水體系更趨混溶，縮小兩相組成差距，降低有效 $KLL$
3. **兩相密度差減小**：丙酮進入兩相導致相組成趨近，兩相平衡濃度差（傳質推動力）因而下降

---

#### 由模擬結果反推實際有效萃取因子 $E_{\text{eff}}$

利用方案一（單一 Decanter）的 Aspen Plus NRTL-LLE 嚴格計算結果，反推本體系在操作濃度下的實際有效萃取因子：

**方案一模擬結果**：RAFFINATE 丙酮 16.84%，EXTRACT 丙酮回收率 = 57.2%（即 $\eta_{\text{actual}} = 0.572$ ）

由單級回收率公式反推有效 $E$ ：

$$
\eta = \frac{E}{1 + E} \Rightarrow E_{\text{eff}} = \frac{\eta}{1 - \eta} = \frac{0.572}{0.428} \approx 1.34
$$

由此反推實際操作濃度下的有效 $KLL$ ：

$$
KLL_{\text{eff}} = \frac{E_{\text{eff}}}{S'/F'} = \frac{1.34}{6.99} \approx 0.19
$$

**表 MSCCE-2 稀溶液預估 vs 實際嚴格計算（Aspen Plus NRTL-LLE）**

| 參數 | 稀溶液極限假設 | 實際操作條件（由模擬反推） |
|:---:|:---:|:---:|
| $KLL_{\text{Acetone}}$ | 1.2（ $\approx 6\times$ 高估） | **≈ 0.19** |
| 萃取因子 $E$ | 8.4 | **≈ 1.34** |
| 方案一回收率（預估） | 89.4% | — |
| 方案一回收率（Aspen 嚴格） | — | **57.2%** |
| 方案三/四回收率（預估） | 99.9% | — |
| 方案三/四回收率（Aspen 嚴格） | — | **82.7%** |

**驗證**：以 $E_{\text{eff}} = 1.34$ 代入 3 板逆流 Kremser 公式：

$$
\eta_{\text{3-stage}} = \frac{E(E^3-1)}{E^4-1} = \frac{1.34 \times (2.406-1)}{3.225-1} = \frac{1.34 \times 1.406}{2.225} \approx 84.6\%
$$

此值（84.6%）已相當接近 Aspen 嚴格計算的 82.7%，確認 $E_{\text{eff}} \approx 1.34$ 是描述本體系在操作濃度下的合理有效萃取因子估計。

> **教學重點**：本案例完美示範了**不能直接用稀溶液 Kremser 公式設計高濃度萃取流程**的核心原則。$KLL^{\infty} = 1.2$ 與 $KLL_{\text{eff}} = 0.19$ 相差近 **6 倍**，造成萃取因子 $E$ 被高估 **6 倍**，預測回收率嚴重偏離實際值。正確的設計流程：**先用 Kremser 公式（以 $E_{\text{eff}}$ ）估算板數範圍，再以 Aspen Plus NRTL-LLE 嚴格計算驗證**。

---

### Aspen Plus 模擬步驟

本案教學邏輯從最基礎的單一接觸器出發，逐步演進至嚴格逐板計算模型，共四種方案依序進行模擬與比較。**所有方案共用相同的組分設定與物性方法**，差異僅在流程圖的萃取配置。
